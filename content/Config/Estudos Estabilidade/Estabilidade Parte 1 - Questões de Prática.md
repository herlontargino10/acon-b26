## Resumo das Fórmulas

| Fórmula      | Equação            |
| ------------ | ------------------ |
| Calado Médio | Cm = (Ca + Cr) / 2 |
| Pontal       | **P = C + BL**     |
|              |                    |

____

**Como encontrar o calado médio quando já tenho os dois calados (avante e ré)?**

| **CM = (Ca + Cr) / 2** |
| ---------------------- |

___

**Como encontrar um dos calados (avante ou ré) quando já tenho o calado médio e o outro calado?**

***Multiplica por 2, subtrai o que tem.***

| 2 × CM − Cr = **Ca** | quando quero o calado **avante** |
| -------------------- | -------------------------------- |
| 2 × CM − Ca = **Cr** | quando quero o calado **à ré**   |

____


---

## Questões

**1.** Uma embarcação possui calado avante de 5,80m e calado à ré de 7,60m. Qual o calado médio?



**2.** Calcule o calado à ré de uma embarcação cujo calado avante é 4,96m e o calado médio é 6,13m.




**3.** Calcule o calado avante de uma embarcação sabendo que o calado à ré vale 9,14m e o calado médio vale 7,55m.




**4.** Os calados avante e à ré de uma embarcação somam 15,40m. Sabendo que o calado à ré é 1,80m maior que o calado avante, determine os dois calados e o calado médio.




**5.** Determine os calados avante e à ré de uma embarcação, sabendo que o calado médio vale 6,75m e o calado avante corresponde à metade do calado à ré.




**6.** Determine o pontal de uma embarcação que possui calado de 5,60m e borda livre de 2,45m.




**7.** Determine a borda livre de uma embarcação cujo pontal é 12,30m e o calado é 8,74m.




**8.** Uma embarcação possui pontal de 9,60m. Sabendo que a borda livre vale um terço do calado, determine o calado e a borda livre.




**9.** Determine o pontal de uma embarcação sabendo que o calado vale o triplo da borda livre e a borda livre é 2,15m.




**10.** Uma embarcação tem calado médio de 7,50m, calado à ré igual ao dobro do calado avante, pontal de 11,20m. Determine os calados avante e à ré, e a borda livre (considere o calado médio para o cálculo da borda livre).




---

## Gabarito Comentado

### Questão 1
- **Fórmula:** Cm = (Ca + Cr) / 2
- **Cálculo:** Cm = (5,80 + 7,60) / 2 = 13,40 / 2
- **Resposta: Cm = 6,70m**

### Questão 2
- **Fórmula:** Ca + Cr = 2 · Cm
- **Cálculo:** 4,96 + Cr = 2 × 6,13 = 12,26 → Cr = 12,26 − 4,96
- **Resposta: Cr = 7,30m**

### Questão 3
- **Fórmula:** Ca + Cr = 2 · Cm
- **Cálculo:** Ca + 9,14 = 2 × 7,55 = 15,10 → Ca = 15,10 − 9,14
- **Resposta: Ca = 5,96m**

### Questão 4
- **Dados:** Ca + Cr = 15,40 e Cr = Ca + 1,80
- **Substituindo:** Ca + (Ca + 1,80) = 15,40 → 2·Ca = 13,60 → Ca = 6,80m
- Cr = 6,80 + 1,80 = 8,60m
- Cm = 15,40 / 2 = 7,70m
- **Resposta: Ca = 6,80m | Cr = 8,60m | Cm = 7,70m**

### Questão 5
- **Dados:** Cm = 6,75 e Ca = Cr / 2
- **Macete:** Ca + Cr = 2 × 6,75 = 13,50
- Substituindo: (Cr/2) + Cr = 13,50 → 3·Cr/2 = 13,50 → Cr = 9,00m
- Ca = 9,00 / 2 = 4,50m
- **Resposta: Ca = 4,50m | Cr = 9,00m**

### Questão 6
- **Fórmula:** P = C + BL
- **Cálculo:** P = 5,60 + 2,45
- **Resposta: P = 8,05m**

### Questão 7
- **Fórmula:** BL = P − C
- **Cálculo:** BL = 12,30 − 8,74
- **Resposta: BL = 3,56m**

### Questão 8
- **Dados:** P = 9,60 e BL = C / 3
- **Substituindo em P = C + BL:** 9,60 = C + C/3 = 4·C/3
- C = 9,60 × 3/4 = 7,20m
- BL = 7,20 / 3 = 2,40m
- **Resposta: C = 7,20m | BL = 2,40m**

### Questão 9
- **Dados:** C = 3 · BL e BL = 2,15m
- C = 3 × 2,15 = 6,45m
- P = C + BL = 6,45 + 2,15
- **Resposta: P = 8,60m**

### Questão 10
- **Parte 1 (Calados):** Cr = 2·Ca e Ca + Cr = 2 × 7,50 = 15,00
- Ca + 2·Ca = 15,00 → 3·Ca = 15,00 → Ca = 5,00m | Cr = 10,00m
- **Parte 2 (Borda livre):** BL = P − Cm = 11,20 − 7,50
- **Resposta: Ca = 5,00m | Cr = 10,00m | BL = 3,70m**
